...
A lightweight, high-performance, and industrial-styled UI for a high-performance inference engine. Designed for advanced character interaction with a minimalist, sharp, and dark aesthetic.

## Features

**Engine Management**: Automatically launch and manage your inference engine process directly from the UI.  
...
- **Roleplay Protocol**: Injects character instructions directly into the dialogue history to maximize persona adherence on instruction-tuned models.  
- **Typography Filter**: Native em-dash (—) filtering to ensure consistent character dialogue style.  
**Advanced Character System**:  
- **PNG Import**: Supports character cards with metadata parsing.  
...
- **Inference Engine Executable** (place in `engine/build/Release/`)  
- **Model Files** (place in `engine/models/`)
...
**Connect**: Toggle the inference engine on/off.  
...
- **Inference**: Custom API or compatible inference engine (OpenAI Compatible)
